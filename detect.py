        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    real_Y = (y_center - 240) * depth_Z / 525

                    # 3. 打印结果
                    # .item() 用于将 Tensor 转换为 Python 浮点数
                    print(f"Found: {names[int(cls)]}")
                    print(f" -> Pixel Center: ({x_center.item():.1f}, {y_center.item():.1f})")
                    print(f" -> Sim 3D Coord: X={real_X.item():.1f}mm, Y={real_Y.item():.1f}mm, Z={depth_Z}mm")
                    # ================== 修改结束 ==================
